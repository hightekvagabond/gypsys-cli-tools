...
# Scan for module-specific hardware configurations
scan_module_hardware() {
    local module_dir="$1"
    local module_name="$(basename "$module_dir")"
    
    # Skip if no scan script exists
    local scan_script="$module_dir/scan.sh"
    if [[ ! -x "$scan_script" ]]; then
        return 0
    fi
    
    print_info "Scanning for $module_name hardware..."
    
    # Run module scan script and capture results
    local scan_results
    if scan_results=$("$scan_script" 2>/dev/null); then
        if [[ -n "$scan_results" ]]; then
            echo -e "${CYAN}📋 $module_name scan results:${NC}"
            echo "$scan_results" | sed 's/^/  /'
            echo ""
            return 0
        fi
    fi
...
# Run hardware scan for investigation/testing
run_hardware_scan() {
    print_info "Scanning hardware configuration..."
    echo ""
    
    # Scan all modules that have scan scripts
    local modules_scanned=0
    local modules_with_results=0
    ...
    if [[ -d "$SCRIPT_DIR/$MODULES_DIR" ]]; then
        for module_dir in "$SCRIPT_DIR/$MODULES_DIR"/*; do
            if [[ -d "$module_dir" ]]; then
                ...
                # Skip test/nonexistent modules
                if [[ "$module_name" == "nonexistent" ]]; then
                    continue
                fi
                
                ((modules_scanned++))
                if scan_module_hardware "$module_dir"; then
    ...
    echo "  Modules with hardware detected: $modules_with_results"
    echo ""
    
    if [[ $modules_with_results -eq 0 ]]; then
        print_warning "No hardware-specific configurations detected"
        print_info "Run 'sudo ./setup.sh' to install with default configuration"
    else
        print_status "Hardware scan completed"
        print_info "Run 'sudo ./setup.sh' to install with auto-detected configuration"
        print_info "Or run 'sudo ./setup.sh --reconfigure' to update existing installation"
    fi
}

# Run hardware scan and apply configuration during setup
run_hardware_scan_and_configure() {
    print_info "Auto-detecting hardware configuration..."
    echo ""
    
    # Create a temporary config file for detected settings
    local temp_config="/tmp/modular-monitor-detected.conf"
    echo "# Auto-detected hardware configuration" > "$temp_config"
    echo "# Generated by setup.sh on $(date)" >> "$temp_config"
    echo "" >> "$temp_config"
    
    local config_applied=false
    
    # Scan all modules and collect configuration
    if [[ -d "$SCRIPT_DIR/$MODULES_DIR" ]]; then
        for module_dir in "$SCRIPT_DIR/$MODULES_DIR"/*; do
            if [[ -d "$module_dir" ]]; then
                local module_name="$(basename "$module_dir")"
                ...
                local scan_script="$module_dir/scan.sh"
                if [[ -x "$scan_script" ]]; then
                    print_info "Auto-detecting $module_name configuration..."
                    
                    # Get scan results in config format
                    local scan_config
                    if scan_config=$("$scan_script" --config 2>/dev/null); then
                        if [[ -n "$scan_config" ]]; then
                            echo "# $module_name module auto-detection" >> "$temp_config"
                            echo "$scan_config" >> "$temp_config"
                            echo "" >> "$temp_config"
                            config_applied=true
                            print_status "$module_name configuration detected"
                        fi
                    fi
                fi
            fi
    ...
    # Apply detected configuration to SYSTEM.conf
    if [[ "$config_applied" == "true" ]]; then
        local system_config="$SCRIPT_DIR/config/SYSTEM.conf"
        print_info "Applying auto-detected configuration to $system_config..."
        
        # Backup existing config
        if [[ -f "$system_config" ]]; then
            cp "$system_config" "${system_config}.backup.$(date +%s)"
        fi
        
        # Merge detected config with existing config
        if [[ -f "$system_config" ]]; then
            # Append new settings (preserving existing ones)
            echo "" >> "$system_config"
            echo "# ========================================" >> "$system_config"
            echo "# AUTO-DETECTED HARDWARE CONFIGURATION" >> "$system_config"
            echo "# ========================================" >> "$system_config"
            cat "$temp_config" | grep -v "^#" | grep -v "^$" >> "$system_config"
        else
            # Create new config file
            cat "$temp_config" > "$system_config"
        fi
        
        print_status "Hardware configuration applied automatically"
    else
        print_info "No hardware-specific configuration detected - using defaults"
    fi
...
main() {
    ...
    if [[ "$scan_only" == "true" ]]; then
        check_requirements "scan"
        print_header
        echo -e "${CYAN}🔍 HARDWARE SCAN MODE${NC}"
        echo ""
        run_hardware_scan
        exit 0
    else
    ...
    if [[ "$reconfigure" == "true" ]]; then
        ...
        run_preflight_tests
        # Auto-scan hardware during normal setup
        run_hardware_scan_and_configure
        configure_modules  
        configure_settings
        complete_installation
