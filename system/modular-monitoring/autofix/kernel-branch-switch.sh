...
get_kernel_package_name() {
    ...
    case "$branch" in
        "stable")
            ...
            ;;
        "longterm")
            # Determine current LTS version
            local lts_release
            lts_release=$(lsb_release -rs 2>/dev/null || echo "22.04")
            if [[ "$lts_release" == "22.04" ]]; then
                echo "linux-image-generic-hwe-22.04"
            elif [[ "$lts_release" == "20.04" ]]; then
                echo "linux-image-generic-hwe-20.04"
            else
                echo "linux-image-generic"  # Fallback to generic
            fi
            ;;
        "mainline")
            echo "linux-image-generic"  # Use generic as mainline fallback
            ;;
        "linux-next")
            echo "linux-image-generic"  # Note: linux-next typically requires manual installation from mainline PPA
            ;;
        *)
...
perform_kernel_branch_switch() {
    ...
    case "$TARGET_BRANCH" in
        "stable"|"longterm"|"mainline"|"linux-next")
            autofix_log "INFO" "Valid target branch: $TARGET_BRANCH"
            ;;
        *)
            autofix_log "ERROR" "Invalid target branch: $TARGET_BRANCH"
            autofix_log "ERROR" "Valid options: stable, longterm, mainline, linux-next"
            return 1
            ;;
    esac
